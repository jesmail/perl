...
Perl__to_upper_title_latin1(pTHX_ const U8 c, U8* p, STRLEN *lenp, const char S_or_s)
{
	...
	*lenp = 1;
    ...
    if (converted == LATIN_SMALL_LETTER_Y_WITH_DIAERESIS) {
	switch (c) {
...
#define CALL_UPPER_CASE(INP, OUTP, LENP) Perl_to_utf8_case(aTHX_ INP, OUTP, LENP, &PL_utf8_toupper, "ToUpper", "utf8::ToSpecUpper")
#define CALL_TITLE_CASE(INP, OUTP, LENP) Perl_to_utf8_case(aTHX_ INP, OUTP, LENP, &PL_utf8_totitle, "ToTitle", "utf8::ToSpecTitle")
#define CALL_LOWER_CASE(INP, OUTP, LENP) Perl_to_utf8_case(aTHX_ INP, OUTP, LENP, &PL_utf8_tolower, "ToLower", "utf8::ToSpecLower")
#define CALL_FOLD_CASE(INP, OUTP, LENP, SPECIALS) Perl_to_utf8_case(aTHX_ INP, OUTP, LENP, &PL_utf8_tofold, "ToFold", (SPECIALS) ? "utf8::ToSpecFold" : NULL)
...
Perl_to_uni_upper(pTHX_ UV c, U8* p, STRLEN *lenp)
{
    ...
    if (c < 256) {
	return _to_upper_title_latin1(c, p, lenp, 'S');
    }
...
Perl_to_uni_lower(pTHX_ UV c, U8* p, STRLEN *lenp)
{
    ...
    if (c < 256) {
	return to_lower_latin1(c, p, lenp);
    }
...
Perl__to_uni_fold_flags(pTHX_ UV c, U8* p, STRLEN *lenp, U8 flags)
{
    ...
    if (c < 256) {
	return _to_fold_latin1(c, p, lenp, flags);
    }
...
Perl_to_utf8_upper(pTHX_ const U8 *p, U8* ustrp, STRLEN *lenp)
{
	...
	return _to_upper_title_latin1(TWO_BYTE_UTF8_TO_UNI(*p, *(p+1)), ustrp, lenp, 'S');
    }
...
Perl__to_utf8_fold_flags(pTHX_ const U8 *p, U8* ustrp, STRLEN *lenp, U8 flags)
{
	...
	return _to_fold_latin1(TWO_BYTE_UTF8_TO_UNI(*p, *(p+1)), ustrp, lenp, flags);
    }
